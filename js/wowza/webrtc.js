...
const GO_BUTTON_START_TEXT = "Share Stream";
const GO_BUTTON_STOP_TEXT = "Stop Stream";
...
var wsURL = "wss://mirichat.com:4443/webrtc-session.json";
...
function pageReady()
{
	...
	//new code for hide/show buttons
	//$('#broadcastingButton').text('');
	//$('#broadcastingButton').text('Stop Camera');
	$('#broadcastingButton').addClass('hideClass');
	$('#buttonGo').removeClass('hideClass');
...
function wsConnect(url)
{
	...
	wsConnection.onmessage = function(evt)
	{
		...
		if (msgStatus != 200)
		{
			$(".sdpDataTagClass").removeClass('hide');
			$("#sdpDataTag").html(msgJSON['statusDescription']);
			stopPublisher();
		}
		else
		{
			$(".sdpDataTagClass").addClass('hide');
			$("#sdpDataTag").html("");
...
function startPublisher()
{
	...
	$("#buttonGo").text('').text(GO_BUTTON_STOP_TEXT);

	console.log('in startPublisher');

	//after this set broadcast status for performer
	setPerformerOnlineStatus('broadcast');
}
...
function stopPublisher()
{
	...
	$("#buttonGo").attr('value', GO_BUTTON_START);
	$("#buttonGo").text('').text(GO_BUTTON_START_TEXT);

	//console.log("stopPublisher");

	//after this set logout status for performer
